-- ============================================================
-- Sensor Tower Market Analysis API - DB Schema (v6)
-- 대상 DB: PostgreSQL
-- 수정일: 2026-04-10
-- v6: v5 대비 변경사항
--     - st_app_detail VARCHAR 길이 확장 (country, publisher_country, content_rating 50자)
--     - idx_app_detail_unified 인덱스 추가
--     - 사용 안 하는 뷰 제거 (v_app_detail_with_genre, v_top_revenue_with_detail)
--     - st_master_apps / st_master_publishers 마스터 레지스트리 유지
--     - st_app_profile(custom_tags JSONB) 유지
--     - 매출은 센트(cents) 단위로 저장
-- ============================================================
...
-- ============================================================
-- 0-2. 마스터 레지스트리 (내부 테이블 UNION으로 빌드, API 호출 없음)
-- ============================================================

-- 고유 앱 마스터 (TOP 100 계열 테이블들에서 UNION)
CREATE TABLE IF NOT EXISTS st_master_apps (
    id              BIGSERIAL PRIMARY KEY,
    os              VARCHAR(10)  NOT NULL,
    app_id          VARCHAR(200) NOT NULL,
    first_seen_date DATE,
    last_seen_date  DATE,
    sources         JSONB,                 -- ['revenue_top100', 'units_top100', 'mau_top100', 'publisher_apps']
    updated_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    UNIQUE (os, app_id)
);

CREATE INDEX IF NOT EXISTS idx_master_apps_sources ON st_master_apps USING GIN (sources);

-- 고유 퍼블리셔 마스터
CREATE TABLE IF NOT EXISTS st_master_publishers (
    id              BIGSERIAL PRIMARY KEY,
    os              VARCHAR(10)  NOT NULL,
    ...
    sources         JSONB,
    updated_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    UNIQUE (os, publisher_id)
);

CREATE INDEX IF NOT EXISTS idx_master_pub_sources ON st_master_publishers USING GIN (sources);

-- ============================================================
-- 1. 앱 상세정보 (/v1/{os}/apps)
--    sales_report, publishers에서 수집된 app_id로 조회
-- ============================================================
