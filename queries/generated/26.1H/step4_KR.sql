...
WITH base AS (
  SELECT EXTRACT(YEAR FROM (date AT TIME ZONE 'Asia/Seoul'))::int AS yr, (date AT TIME ZONE 'Asia/Seoul') AS d, revenue_krw_100 AS krw,
         CASE WHEN publisher_name ILIKE '%FUNFLY%' THEN '중화권' WHEN publisher_name ILIKE '%NEXON%' THEN 'KR' WHEN publisher_country IN ('China','Hong Kong','Taiwan','Macao') THEN '중화권' WHEN publisher_country IN ('US','Canada') THEN '북미' WHEN publisher_country='Japan' THEN 'JP' WHEN publisher_country='South Korea' THEN 'KR' ELSE '기타' END AS nat,
         COALESCE(lv2_genre, sub_genre, genre) AS eg
  FROM dw_app_monthly
...
FROM base
GROUP BY nat, eg
HAVING COALESCE(ROUND(SUM(krw) FILTER (WHERE yr IN (2022,2023,2024))/36/1e8),0)>=30 OR COALESCE(ROUND((SUM(krw) FILTER (WHERE yr IN (2025))+COALESCE(SUM(krw) FILTER (WHERE d>='2026-01-01' AND d<'2026-07-01'),0))/18/1e8),0)>=30
ORDER BY chg DESC;
