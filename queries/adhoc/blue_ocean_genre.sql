-- ============================================================
-- 블루오션 장르 식별 쿼리  (애드혹 — 분기 보고서용 아님)
-- 조건: 매출 규모 + 낮은 HHI(경쟁 분산) + 높은 YoY 성장률
-- 테이블: dw_app_monthly
-- 기간: 최근 1년(2025-04 ~ 2026-03) vs 전년(2024-04 ~ 2025-03)
-- 범위: 전체 국가, 전체 OS 합산
-- 작성: 2026-04, 신규 장르 후보 탐색용 1회성 쿼리
--
-- ⚠️ 이 쿼리는 분기 보고서 집계 표준을 따르지 않습니다. 그대로 재사용하지 마세요.
--    - revenue_usd 사용        → 표준은 revenue_krw_100 (환율 반영 완료 컬럼)
--    - TOP100 필터 없음        → 표준은 in_revenue_top100_unified_os = TRUE
--    - app_id 로 집계          → 표준은 unified_app_id (OS 중복 제거)
--    - 원본 genre 사용         → 표준은 effective_genre (lv2 특수버킷 우선)
--    - 기간 하드코딩
--    보고서 수치를 뽑을 때는 queries/generated/ 를 쓰세요. → docs/QUARTERLY.md
-- ============================================================
