...
--  출처: verify/verify_kr_top5.py  (SQL을 실행 시점에 추출)
...
-- ── [1/2] ──────────────────────────
SELECT ROUND(SUM(revenue_krw_100)/100000000.0/12, 0) as monthly_avg_억
FROM dw_app_monthly
WHERE country='KR'
  AND in_revenue_top100_unified_os=TRUE
  AND date >= '2022-01-01' AND date < '2023-01-01'
  AND (publisher_name ILIKE '%NEXON%' OR publisher_name ILIKE '%nexon%')
