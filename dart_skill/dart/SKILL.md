...
- **출처**: DART XBRL(22~25) + DART API + 전체공시 + 수동 정리 + 넥슨·그라비티 해외공시 수동
- **대상**: 한국 게임사 **28개** (v3: 넥슨·그라비티 추가 / 골프존·넥슨지티·스코넥·컴투스홀딩스 제외)
- **기간**: 2022~2025 (4개년)
...
NHN, 네오위즈, 넥슨, 넥슨게임즈, 넵튠, 넷마블, 더블유게임즈, 데브시스터즈, 드래곤플라이, 모비릭스, 미투온, 시프트업, 썸에이지, 액토즈소프트, 엔씨소프트, 엠게임, 웹젠, 위메이드, 위메이드맥스, 위메이드플레이, 조이시티, 카카오게임즈, 컴투스, 크래프톤, 펄어비스, 플레이위드, 한빛소프트, 그라비티

### 특수 corp_code (해외상장사)

| 회사 | corp_code |
|---|---|
| 넥슨 | `해외상장_일본도쿄증시_3659.T (Nexon Co., Ltd.)` |
| 그라비티 | `해외상장_미국나스닥_GRVY (Gravity Co., Ltd.)` |

해외상장사는 DART 고유번호가 없어 위 형태의 긴 문자열로 저장됨. `corp_code` 컬럼은 **VARCHAR(64)**.
...
| 컬럼 | 타입 | 설명 |
|---|---|---|
| `corp_code` (PK) | VARCHAR(64) | DART 고유번호 (해외상장사는 긴 placeholder) |
...
| labor_cost | 인건비 |
| advertising_expense | 광고선전비 |
| **cash_and_equivalents** | **현금성자산** (v3 신규, 현금및현금성자산 + 단기금융상품 + 단기투자자산 등) |
...
- **cash_and_equivalents**: v3 신규. 현금및현금성자산 외에 단기금융상품·단기투자자산까지 포함한 확장 개념. `_추정` 플래그와 별도로 수동/PDF에서 추출한 값. 2026-04 기준 448건 중 438건 채워짐 (97.8%)

**회사별 예외사항**:
- **카카오게임즈 2024년**: 연중 연결범위 변경으로 분기 XBRL과 사업보고서 수치가 불일치하여, 사업보고서 기준 수치로 수동 반영
- **조이시티 2025년**: 연간 매출액 원본 데이터 오류로 분기 합산 기준(1,404억)으로 수정
- **데브시스터즈, 조이시티**: XBRL에 `ifrs-full:Revenue` 태그가 없어 `ifrs-full:GrossProfit`(영업수익) 태그를 매출액으로 사용
- **넥슨, 그라비티**: 해외공시(일본·미국) 수동 반영. `corp_code`가 긴 placeholder 문자열
...
| 넥슨 | NEXON Company / NEXON Co., Ltd. | v3 추가, 해외상장 |
...
| 그라비티 | GRAVITY Co., Ltd. | v3 추가, 해외상장 (매핑 가능, 확인 필요) |

- **매핑 완료**: ~22개사 (넥슨/그라비티 매핑 테이블 추가 필요 시 반영)
- **매핑 불가**: 5개사 (넥슨게임즈, 미투온, 시프트업, 썸에이지, 액토즈소프트)
- **삭제된 회사** (v3): 골프존, 넥슨지티, 스코넥, 컴투스홀딩스
